...
### B. Create
- 입력: `topic`(필수), `items` 8개/16개/32개(필수, 1줄 1후보)
- 검증:
  - 공백/빈 줄 제거
  - 8개/16개/32개 외 개수는 에러
...
- 옵션:
  ...
  - 셔플(기본 ON)
  - 시드 고정(기본 ON 권장)
- 이미지(선택): 후보별 로컬 이미지 첨부 (자세한 내용은 5.1 로컬 이미지 처리 스펙 참조)
- `시작하기` 버튼은 검증 통과 시 활성화

### C. Match
- 헤더: 주제, 라운드(32강/16강/8강/준결승/결승 중 현재 단계), 진행률
- 라운드 전환 애니메이션: 각 단계 시작 시 전체화면 오버레이로 단계명(32강/16강/8강/준결승/결승) 표시 후 자동 페이드아웃
- 후보 카드 2개 표시
- 조작:
  - 카드 클릭/탭으로 승자 선택
  - 단축키: `1`/`2`, `←`/`→`
...
type Tournament = {
  ...
  createdAt: number;
  updatedAt: number;
  items: Item[]; // length 8 | 16 | 32
  rounds: Partial<Record<32 | 16 | 8 | 4 | 2, Match[]>>; // 시작 라운드보다 큰 키는 없음(예: 8강 시작 시 32/16 라운드 없음)
  cursor: {
    round: 32 | 16 | 8 | 4 | 2;
    matchIndex: number;
  };
...
- 방식:
  - payload(JSON): `topic`, `items[8 | 16 | 32]`, `seed`
