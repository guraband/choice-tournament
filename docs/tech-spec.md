...
- 이미지(선택): 후보별 로컬 이미지 첨부
  ...
  - 저장 포맷: base64 문자열(권장: data URL 형식 또는 mime prefix 제거 문자열)
  - 권장 인코딩: `image/webp` 품질 0.8 (미지원 브라우저는 JPEG 폴백)
- `시작하기` 버튼은 검증 통과 시 활성화

### C. Match
- 헤더: 주제, 라운드(32강/16강/8강/4강/결승 중 현재 단계), 진행률
- 후보 카드 2개 표시
- 조작:
  - 카드 클릭/탭으로 승자 선택
  - 단축키: `1`/`2`, `←`/`→`
...
type Tournament = {
  ...
  rounds: Partial<Record<32 | 16 | 8 | 4 | 2, Match[]>>; // 16강 시작 시 32 라운드는 없음
  cursor: {
    round: 32 | 16 | 8 | 4 | 2;
    matchIndex: number;
  };
...
- 방식:
  - payload(JSON): `topic`, `items[16 | 32]`, `seed`
